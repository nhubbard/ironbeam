...
//! therefore has no static knowledge of `T`. Rust has no runtime reflection
...
//! length-prefixed halves, matching Beam's `kv<lp, lp>` wire coder.
...
/// Turns a single element's bincode bytes into a one-element `Box<Vec<T>>`
...
pub trait ElementCoder: Send + Sync {
    /// Wrap one bincode-encoded element as a `Partition` of `Vec<T>` holding a
    /// single element.
    fn decode_one(&self, bytes: &[u8]) -> Result<Partition>;

    /// Downcast `partition` to `Vec<T>` and bincode-encode every element,
    ...
    /// `true` for KV-aware coders (see [`BincodeKvCoder`]). Lets a backend
...
/// Default coder: pairs `T` with bincode. Deterministic on the wire and matches
/// ironbeam's `Partition = Box<Vec<T>>` shape.
pub struct BincodeCoder<T>(PhantomData<fn() -> T>);
...
impl<T> ElementCoder for BincodeCoder<T>
where
    T: Serialize + DeserializeOwned + 'static + Send + Sync + Clone,
{
    fn decode_one(&self, bytes: &[u8]) -> Result<Partition> {
        let v: T = bincode::deserialize(bytes)
            .with_context(|| format!("bincode decode into {}", std::any::type_name::<T>()))?;
        Ok(Box::new(vec![v]))
    }
    ...
    fn encode_all(&self, partition: Partition) -> Result<Vec<Vec<u8>>> {
        ...
        for v in vec.into_iter() {
            out.push(
                bincode::serialize(&v)
                    .with_context(|| format!("bincode encode {}", std::any::type_name::<T>()))?,
            );
        }
        Ok(out)
    }
...
/// KV-aware coder. Wire-identical to `BincodeCoder<(K, V)>` for the in-bundle
/// paths (`decode_one`/`encode_all` round-trip a bincode tuple), but also
/// implements [`ElementCoder::encode_kv_pairs`], returning the `K` and `V`
/// halves encoded separately. The pre-`GroupByKey` node carries this so the
/// runner can read each half as independently length-prefixed bincode, the
/// `kv<lp<bincode>, lp<bincode>>` shape the translator emits there.
pub struct BincodeKvCoder<K, V>(PhantomData<fn() -> (K, V)>);

impl<K, V> BincodeKvCoder<K, V> {
    #[must_use]
    pub fn new() -> Self {
        Self(PhantomData)
    }
...
impl<K, V> Default for BincodeKvCoder<K, V> {
    fn default() -> Self {
        Self::new()
    }
}

impl<K, V> ElementCoder for BincodeKvCoder<K, V>
where
    K: Serialize + DeserializeOwned + 'static + Send + Sync + Clone,
    V: Serialize + DeserializeOwned + 'static + Send + Sync + Clone,
{
    fn decode_one(&self, bytes: &[u8]) -> Result<Partition> {
        let pair: (K, V) = bincode::deserialize(bytes)
            .with_context(|| format!("bincode decode into {}", std::any::type_name::<(K, V)>()))?;
        Ok(Box::new(vec![pair]))
    }
    ...
    fn encode_all(&self, partition: Partition) -> Result<Vec<Vec<u8>>> {
        ...
        for pair in vec.into_iter() {
            out.push(
                bincode::serialize(&pair).with_context(|| {
                    format!("bincode encode {}", std::any::type_name::<(K, V)>())
                })?,
            );
        }
        Ok(out)
    ...
    fn encode_kv_pairs(&self, partition: Partition) -> Result<Vec<(Vec<u8>, Vec<u8>)>> {
        ...
        for (k, v) in vec.into_iter() {
            let kb = bincode::serialize(&k)
                .with_context(|| format!("bincode encode key {}", std::any::type_name::<K>()))?;
            let vb = bincode::serialize(&v)
                .with_context(|| format!("bincode encode value {}", std::any::type_name::<V>()))?;
            out.push((kb, vb));
        }
        Ok(out)
    }
...
mod tests {
    ...
    #[test]
    fn bincode_coder_roundtrip_u64() {
        let c = BincodeCoder::<u64>::new();
        let bytes = bincode::serialize(&42u64).unwrap();
    ...
    fn kv_coder_encodes_halves_separately() {
        let c = BincodeKvCoder::<String, u64>::new();
        let partition: Partition =
            Box::new(vec![("alice".to_string(), 1u64), ("bob".to_string(), 2u64)]);
        assert!(c.is_kv());
        let pairs = c.encode_kv_pairs(partition).unwrap();
        assert_eq!(pairs.len(), 2);
        assert_eq!(bincode::deserialize::<String>(&pairs[0].0).unwrap(), "alice");
        assert_eq!(bincode::deserialize::<u64>(&pairs[0].1).unwrap(), 1);
    }

    #[test]
    fn kv_coder_intermediate_tuple_roundtrip_matches_bincode() {
        let c = BincodeKvCoder::<String, u64>::new();
    ...
    fn plain_coder_rejects_kv_encode() {
        let c = BincodeCoder::<(String, u64)>::new();
