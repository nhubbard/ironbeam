...
pub fn assert_collections_equal<T: Debug + PartialEq>(actual: &[T], expected: &[T]) {
    assert_eq!(actual.len(), expected.len(), "Collection length mismatch:\n  Expected length: {}\n  Actual length: {}\n  Expected: {expected:?}\n  Actual: {actual:?}", expected.len(), actual.len());

    for (i, (a, e)) in actual.iter().zip(expected.iter()).enumerate() {
        assert_eq!(a, e, "Collection mismatch at index {i}:\n  Expected: {e:?}\n  Actual: {a:?}\n  Full expected: {expected:?}\n  Full actual: {actual:?}");
    }
}
...
/// ```
pub fn assert_collections_unordered_equal<T: Debug + Eq + Hash>(
    actual: &[T],
    expected: &[T],
) {
...
/// ```
pub fn assert_grouped_kv_equal<K, V>(
    mut actual: Vec<(K, Vec<V>)>,
    mut expected: Vec<(K, Vec<V>)>,
) where
    K: Debug + Ord,
    V: Debug + Eq + Hash,
{
    ...
    assert_eq!(actual.len(), expected.len(), "Collection length mismatch:\n  Expected length: {}\n  Actual length: {}", expected.len(), actual.len());
    ...
    for (i, ((ak, av), (ek, ev))) in actual.iter().zip(expected.iter()).enumerate() {
        ...
        assert_eq!(av_set, ev_set, "Value mismatch for key {ak:?} at index {i}:\n  Expected values: {ev:?}\n  Actual values: {av:?}");
    }
}
...
pub fn assert_any<T: Debug>(collection: &[T], predicate: impl Fn(&T) -> bool) {
    assert!(collection.iter().any(&predicate), 
...
pub fn assert_collection_size<T>(collection: &[T], expected_size: usize) {
    assert_eq!(collection.len(), expected_size, "Collection size mismatch:\n  Expected: {expected_size}\n  Actual: {}", collection.len());
}
...
where
    ...
    V: Debug + PartialEq,
{
    assert_eq!(actual.len(), expected.len(), "HashMap size mismatch:\n  Expected size: {}\n  Actual size: {}\n  Expected: {expected:?}\n  Actual: {actual:?}", expected.len(), actual.len());
