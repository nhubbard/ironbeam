...
fn sum_min_max_average_basic_and_lifted() -> Result<()> {
    ...
    assert_kv_collections_equal(sum_direct, sum_lifted);
    ...
    let min_lifted = from_vec(&p, vals.clone())
        ...
        .collect_par_sorted_by_key(Some(4), None)?;
    assert_kv_collections_equal(min_direct, min_lifted);
    ...
    let max_lifted = from_vec(&p, vals)
        ...
        .collect_par_sorted_by_key(Some(4), None)?;
    assert_kv_collections_equal(max_direct, max_lifted);
    ...
    let avg_lifted = from_vec(&p, (1u32..=100).collect::<Vec<_>>())
        ...
        .collect_par_sorted_by_key(Some(4), None)?;
    assert_kv_collections_equal(avg_direct, avg_lifted);
...
fn distinct_count_basic_and_lifted() -> Result<()> {
    ...
    assert_kv_collections_equal(dc_direct, dc_lifted);
    Ok(())
}
