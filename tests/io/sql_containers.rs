//! Postgres and MySQL integration tests for the SQL I/O connector (feature `io-sql`), run
//! against real database servers started via Testcontainers.
//!
//! Unlike `tests/io/sql.rs` (SQLite, no external process needed), these tests validate
//! the `sqlx::Any` driver's cross-database dispatch against real network databases.
//!
//! Both `postgres:11-alpine` and `mysql:8.1` (the default images pulled by
//! `testcontainers_modules::postgres::Postgres` and `testcontainers_modules::mysql::Mysql`)
//! publish multi-arch manifests covering `linux/amd64` and `linux/arm64`, so these tests
...
//! convention used for the MongoDB Testcontainers suite (see `tests/io/mongodb.rs`). CI runs
//! these with Docker available as a service.
...
fn start_postgres_with_table() -> Result<(testcontainers::Container<Postgres>, String)> {
    ...
    create_table(
        &url,
        "CREATE TABLE t (id BIGINT, name TEXT, value DOUBLE PRECISION)",
    )?;
    Ok((node, url))
}

/// Start a MySQL container and return it (kept alive for the container's lifetime) along
/// with a connection URL, with `t (id, name, value)` already created.
fn start_mysql_with_table() -> Result<(testcontainers::Container<Mysql>, String)> {
    ...
    create_table(&url, "CREATE TABLE t (id BIGINT, name TEXT, value DOUBLE)")?;
    Ok((node, url))
}
...
#[test]
fn postgres_write_then_read_roundtrip_postgresql_scheme() -> Result<()> {
    let (_node, url) = start_postgres_with_table()?;
...
#[test]
fn mysql_write_then_read_roundtrip() -> Result<()> {
    let (_node, url) = start_mysql_with_table()?;
    let data = sample_rows(5);
...
#[test]
fn mysql_streaming_helper_roundtrip() -> Result<()> {
    let (_node, url) = start_mysql_with_table()?;
    let data = sample_rows(7);
    write_sql_with(&url, "INSERT INTO t (id, name, value)", &data, bind_row)?;
...
#[test]
fn mysql_read_sql_helper_roundtrip() -> Result<()> {
    let (_node, url) = start_mysql_with_table()?;
    let data = sample_rows(4);
...
#[cfg(feature = "parallel-io")]
#[test]
fn mysql_write_par_roundtrip() -> Result<()> {
    let (_node, url) = start_mysql_with_table()?;
    let data = sample_rows(20);
