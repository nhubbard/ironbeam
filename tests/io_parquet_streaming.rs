...
use rustflow::{Pipeline, from_vec, read_parquet_streaming, Count};
...
fn parquet_streaming_roundtrip() -> anyhow::Result<()> {
    ...
    let rows: Vec<Rec> = (0..1000).map(|i| Rec { id: i, word: format!("w{}", i % 7) }).collect();
    ...
    let words = s.key_by(|r: &Rec| r.word.clone()).map_values(|_r: &Rec| 1u64).combine_values(Count);
