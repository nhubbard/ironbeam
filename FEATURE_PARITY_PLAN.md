...
| 4.11 Named Operations                       | `PCollection::with_name(name)` fluent post-fix labeller + `Pipeline::set_node_name` / `node_name` / `node_names_snapshot` accessors backed by a per-`Pipeline` `HashMap<NodeId, String>`; names propagate into `Plan::node_names` / `ExecutionExplanation::node_names` and surface in a deterministic "NAMED OPERATIONS" footer in the explain output. Per-step name annotation and `named_scope(...)` composite naming remain follow-ups | 3.1.0  |
