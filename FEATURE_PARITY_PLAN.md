...
| 3.14 Adaptive Inter-Stage Partition Count | `DynOp::cardinality_multiplier_hint() -> f64` (default 1.0); `exec_par` tracks `current_parts` updated per barrier (GBK→×0.1, CombineGlobal→1, Flatten→×N, CoGroup→×0.5); Reshuffle uses `current_parts`; `AdaptivePartitionCount { barrier_count }` opt | 2.14.0 |
